...
func TestParseGGUF_DenseModel(t *testing.T) {
	...
	if meta.Architecture != "llama" { t.Errorf("arch = %q, want llama", meta.Architecture) }
	if meta.Name != "LLaMA 7B" { t.Errorf("name = %q", meta.Name) }
	if meta.ContextLength != 4096 { t.Errorf("context = %d", meta.ContextLength) }
	if meta.QuantName != "Q4_K_M" { t.Errorf("quant = %q", meta.QuantName) }
	if !meta.HasChatTemplate { t.Error("expected HasChatTemplate = true") }
	if meta.IsMoE { t.Error("expected IsMoE = false") }
	if meta.EmbeddingLength != 4096 { t.Errorf("embedding = %d", meta.EmbeddingLength) }
	if meta.BlockCount != 32 { t.Errorf("blocks = %d", meta.BlockCount) }
	if meta.HeadCount != 32 { t.Errorf("heads = %d", meta.HeadCount) }
	if meta.Version != 3 { t.Errorf("version = %d", meta.Version) }
}
...
func TestParseGGUF_MoEModel(t *testing.T) {
	...
	meta, err := ParseGGUF(path)
	if err != nil { t.Fatal(err) }
	if meta.Architecture != "gemma4" { t.Errorf("arch = %q", meta.Architecture) }
	if !meta.IsMoE { t.Error("expected IsMoE = true") }
	if meta.ExpertCount != 64 { t.Errorf("expert_count = %d", meta.ExpertCount) }
	if meta.ExpertUsedCount != 2 { t.Errorf("expert_used_count = %d", meta.ExpertUsedCount) }
	if meta.ContextLength != 131072 { t.Errorf("context = %d", meta.ContextLength) }
	if !meta.HasChatTemplate { t.Error("expected HasChatTemplate = true") }
	qname := meta.QuantDisplayName("gemma-4-26B-A4B-Q3_K_XL.gguf")
	if qname != "Q3_K_XL" { t.Errorf("QuantDisplayName = %q", qname) }
	display := meta.ArchDisplayName()
	if display != "Gemma4 (MoE, 64 experts, 2 active)" { t.Errorf("ArchDisplayName = %q", display) }
}
...
func TestParseGGUF_NoTemplate(t *testing.T) {
	...
	meta, err := ParseGGUF(path)
	if err != nil { t.Fatal(err) }
	if meta.HasChatTemplate { t.Error("expected HasChatTemplate = false") }
	if meta.QuantName != "Q4_0" { t.Errorf("quant = %q", meta.QuantName) }
}
...
func TestParseGGUF_BF16(t *testing.T) {
	...
	meta, err := ParseGGUF(path)
	if err != nil { t.Fatal(err) }
	if meta.QuantName != "BF16" { t.Errorf("quant = %q", meta.QuantName) }
}
...
func TestParseGGUF_NotGGUF(t *testing.T) {
	...
	if err == nil { t.Error("expected error for non-GGUF file") }
}
...
func TestParseGGUF_ArchContextFallback(t *testing.T) {
	...
	meta, err := ParseGGUF(path)
	if err != nil { t.Fatal(err) }
	if meta.ContextLength != 32768 { t.Errorf("context = %d, want 32768", meta.ContextLength) }
}